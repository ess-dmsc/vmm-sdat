...
struct __attribute__ ((packed)) ReadoutSRS {
  ...
  bool operator==(const ReadoutSRS &other) const {
    return (
        (fec == other.fec) &&
...
namespace datatype {
template<>
class TypeTrait<Gem::ReadoutSRS> {
public:
  H5_COMPOUND_DEFINE_TYPE(Gem::ReadoutSRS) {
    H5_COMPOUND_INIT;
...
using ReadoutFile = DumpFile<ReadoutSRS>;

...
void clusterTriggerMatchingSize() {

    vector<double> sizeClusters;
    ...
    treeClusters->SetBranchAddress("clusters_plane.size", &clustersSize);
	...
	if ((int)clustersDet == 0) {
	    ...
	    clusters.push_back(make_pair(clustersTime, clustersSize));
	...
	sizeClusters.push_back(match.second[i].second);
    ...
    TH1D *histFull = new TH1D("histFull", "", 30, 0.0, 30.0);
    TH1D *histMatched = new TH1D("histMatched", "", 30, 0.0, 30.0);
	...
	histFull->Fill(clusters[i].second);
	
    }

    for (int i = 0; i < sizeMatched; i++) {

	histMatched->Fill(sizeClusters[i], 100.0);
	
    }

    histFull->SetLineColor(kAzure+2);
    histMatched->SetLineColor(kPink-1);
    histFull->GetXaxis()->SetTitle("Cluster size / strips [1.2 mm]");
    histFull->GetXaxis()->SetLimits(0.0, 30.0);
    histFull->GetXaxis()->CenterTitle(true);
    //histFull->GetYaxis()->SetTitle("Counts");
    //histFull->GetYaxis()->SetRangeUser(1.0, 1.0e6);
    histFull->GetYaxis()->SetTitle("Counts");
    histFull->GetYaxis()->SetRangeUser(0.0, 8.0e4);
    ...
    c->Update();
    c->SaveAs("clusterTriggerMatchingSize.png");
    c->SaveAs("clusterTriggerMatchingSize.pdf");
